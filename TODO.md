...
Maintenu à jour en cours de session. À appliquer en fin de session ou à la
prochaine session dédiée au skill.

---

## Terminé (session 2026-03-17)

- [x] Renommer "claim" → "affirmation" partout (DESIGN.md appendice)
- [x] Renommer statuts grounded → étayé (DESIGN.md appendice)
- [x] Supprimer "Préparation d'entretien" des axes d'évolution DESIGN.md
      (implémenté)
- [x] Passe d'étayage §2.6 — bloc visible avant génération, critère
      dans grille §3.4
- [x] Restructure Phase 4 — retours en langage naturel §4.1, entretien
      = prep + debrief §4.2, feedback-tracking.md allégé
- [x] Créer references/interview-prep.md — préparation entretien +
      négociation salariale étayée (Ames & Mason 2015)
...
- D-12 : Archivage artefacts — résumé structuré en mémoire `candidature:`
  (pas texte intégral). `conversation_search` pour retrouver le texte si
  besoin.
...
- D-16 : Deux skills complémentaires — `candidature` (claude.ai, grand
  public) et `candidature-desktop` (Claude Desktop, automatisation
  navigateur). Séparation confirmée.
- D-17 : Canal de remontée vers le repo — reporté. Mentionner dans axes
  d'évolution DESIGN.md.
...
- [ ] Mettre à jour le tableau des portes SKILL.md (certaines sont résolues)

### SKILL.md

- [ ] Vérifier que §2.2 mentionne le benchmark salarial comme dimension
      de la recherche contextuelle (pas procédure séparée §2.6)
...
| §2.6 Après génération+relecture | `candidature:` | `replace` (résumé structuré) | `[outil]` |
...
- [ ] Ajouter : candidature-desktop (séparation confirmée)
- [ ] Ajouter : canal de remontée vers le repo (reporté, à concevoir)
- [ ] Ajouter : simulation d'entretien (extension naturelle de
      interview-prep.md)

### README.md

- [ ] Vérifier que les instructions d'installation sont à jour
- [ ] Ajouter interview-prep.md à la description de la structure
